...
contract ProductStore is ERC721 {
    ...
    mapping(address => uint256) public productBalances;

    event ProductPurchased(address indexed buyer, uint256 productId, uint256 amountPaid);

    constructor(ERC20 _token) ERC721("ProductStore", "PS") {
        token = _token;
    }
    ...
    function buyProduct(uint256 productId) external {
        ...
        require(productBalances[msg.sender] >= product.price, "Insufficient funds");

        // Transfer ERC20 tokens from buyer to the contract
        token.transferFrom(msg.sender, address(this), product.price);
        // Update product balance for the buyer
        productBalances[msg.sender] -= product.price;
        // Mint NFT as a bill for the buyer
